 ...
 *   +10  → 20 / 30
 */
export const OBELISK_BUFF_TABLE: ReadonlyArray<{ minLevel: number } & ObeliskBuffValues> = [
 ...
 * to the top tier.
 */
export function getObeliskBuffValues(enhancementLevel: number): ObeliskBuffValues | null {
  if (enhancementLevel < 1) return null;
  const clamped = Math.min(enhancementLevel, 10);
  let values: ObeliskBuffValues | null = null;
  for (const tier of OBELISK_BUFF_TABLE) {
