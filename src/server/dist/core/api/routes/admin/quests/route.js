...
exports.QUEST_SCHEMA = void 0;
exports.createQuestsRouter = createQuestsRouter;
...
function createQuestsRouter(services) {
    const router = (0, express_1.Router)();
    const questSys = services.questSys;
    router.get('/schema', (_req, res) => {
        res.json(exports.QUEST_SCHEMA);
    });
    router.get('/', (req, res) => {
        let quests = questSys.getAllQuestDefinitions();
        const npcFilter = req.query.npc;
        ...
        if (npcFilter)
            quests = quests.filter(q => q.npcId === npcFilter);
        if (typeFilter)
            quests = quests.filter(q => q.type === typeFilter);
        if (typeof minLevel === 'number' && !Number.isNaN(minLevel))
            quests = quests.filter(q => q.requiredLevel >= minLevel);
        if (typeof maxLevel === 'number' && !Number.isNaN(maxLevel))
            quests = quests.filter(q => q.requiredLevel <= maxLevel);
