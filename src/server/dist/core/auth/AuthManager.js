...
class AuthManager {
    constructor() {
        ...
        this.mockCharacters = new Map();
        this.mockPlayerRoles = new Map();
    ...
    async register(username, email, password) {
        if (!this.db.isPostgresConnected()) {
            ...
            if (!player) {
                player = { id: (0, uuid_1.v4)(), username };
        ...
        try {
            const existing = await this.db.postgres.query('SELECT id FROM players WHERE username = $1 OR email = $2', [username, email]);
            if (existing.rows.length > 0) {
                return { success: false, error: 'Username or email already exists' };
            }
            const passwordHash = await bcrypt_1.default.hash(password, 10);
            const result = await this.db.postgres.query('INSERT INTO players (username, email, password_hash) VALUES ($1, $2, $3) RETURNING id, username', [username, email, passwordHash]);
    ...
    async login(username, password) {
        if (!this.db.isPostgresConnected()) {
            ...
            if (!player) {
                ...
                this.mockPlayers.set(username, player);
                this.mockCharacters.set(player.id, []);
                this.mockPlayerRoles.set(player.id, shared_1.AccountRole.PLAYER);
            }
            const role = this.mockPlayerRoles.get(player.id) || shared_1.AccountRole.PLAYER;
            return {
                success: true,
                playerId: player.id,
                username,
        ...
        try {
            const result = await this.db.postgres.query('SELECT id, username, password_hash, level, role FROM players WHERE username = $1', [username]);
            if (result.rows.length === 0) {
                return { success: false, error: 'Invalid username or password' };
            }
            const player = result.rows[0];
            const validPassword = await bcrypt_1.default.compare(password, player.password_hash);
            if (!validPassword) {
                return { success: false, error: 'Invalid username or password' };
            }
    ...
    async getAccountRole(playerId) {
        if (!this.db.isPostgresConnected()) {
            return this.mockPlayerRoles.get(playerId) || shared_1.AccountRole.PLAYER;
        }
        try {
            const result = await this.db.postgres.query('SELECT role FROM players WHERE id = $1', [playerId]);
            if (result.rows.length === 0)
                return shared_1.AccountRole.PLAYER;
    ...
    async setAccountRole(playerId, role) {
        if (!this.db.isPostgresConnected()) {
            this.mockPlayerRoles.set(playerId, role);
            return true;
        }
        try {
            const result = await this.db.postgres.query('UPDATE players SET role = $1 WHERE id = $2', [role, playerId]);
    ...
    async getAccountIdByUsername(username) {
        if (!this.db.isPostgresConnected()) {
    ...
    normalizeRole(raw) {
        if (typeof raw !== 'string')
            return shared_1.AccountRole.PLAYER;
        return Object.values(shared_1.AccountRole).includes(raw) ? raw : shared_1.AccountRole.PLAYER;
    }
    verifyToken(token) {
        try {
    ...
    async createCharacter(playerId, name, race, jobId, racialPassive) {
        ...
        try {
            const existing = await this.db.postgres.query('SELECT id FROM characters WHERE player_id = $1 AND name = $2', [playerId, name]);
            if (existing.rows.length > 0) {
                return { success: false, error: 'Character name already exists' };
            }
            const defaultStats = (0, shared_2.createDefaultStatPoints)();
    ...
    async deleteCharacter(playerId, characterId) {
        if (!this.db.isPostgresConnected()) {
            const characters = this.mockCharacters.get(playerId);
            if (characters) {
        ...
        try {
            await this.db.postgres.query('DELETE FROM characters WHERE id = $1 AND player_id = $2', [characterId, playerId]);
            return true;
        }
        catch (error) {
    ...
    async saveCharacter(characterId, data) {
            ...
            for (const [, chars] of this.mockCharacters) {
                ...
                if (ch) {
                    ...
                    if (data.nation !== undefined)
                        ch.nation = data.nation;
                    if (data.lastSafeZoneId !== undefined)
                        ch.last_safe_zone_id = data.lastSafeZoneId;
                    if (data.inventory !== undefined)
                        ch.inventory = JSON.stringify(data.inventory);
                    if (data.equipment !== undefined)
                        ch.equipment = JSON.stringify(data.equipment);
                    if (data.gold !== undefined)
                        ch.gold = data.gold;
                    if (data.quests !== undefined)
                        ch.character_quests = JSON.stringify(data.quests);
                    if (data.recipes !== undefined)
                        ch.character_recipes = JSON.stringify(data.recipes);
                    if (data.unlockedZones !== undefined)
                        ch.character_unlocked_zones = JSON.stringify(data.unlockedZones);
                    break;
                }
            }
            return;
        ...
        try {
            await this.db.postgres.query(`UPDATE characters SET
         ...
         WHERE id = $13`, [
                ...
                JSON.stringify(data.equipment || { ...shared_2.DEFAULT_EQUIPMENT }),
                data.gold ?? 100,
    ...
    generateToken(playerId, username) {
        return jsonwebtoken_1.default.sign({ playerId, username }, JWT_SECRET, { expiresIn: JWT_EXPIRY });
    }
}
