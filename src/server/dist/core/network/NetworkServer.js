...
const CombatSystem_1 = require("../ecs/systems/CombatSystem");
const AISystem_1 = require("../ecs/systems/AISystem");
const LootSystem_1 = require("../ecs/systems/LootSystem");
...
const SummonManager_1 = require("../world/SummonManager");
const DummyManager_1 = require("../world/DummyManager");
const SpatialIndexManager_1 = require("../world/SpatialIndexManager");
const AOEZoneManager_1 = require("../world/AOEZoneManager");
const SongProximityManager_1 = require("../world/SongProximityManager");
const BlockingProximityManager_1 = require("../world/BlockingProximityManager");
const KnockbackManager_1 = require("../world/KnockbackManager");
const MovementThrottle_1 = require("../world/MovementThrottle");
const ZoneInstance_1 = require("../world/ZoneInstance");
const QuestSystem_1 = require("../../systems/QuestSystem");
const CraftSystem_1 = require("../../systems/CraftSystem");
const CutsceneSystem_1 = require("../../systems/CutsceneSystem");
const ItemSystem_1 = require("../../systems/ItemSystem");
const PresenceService_1 = require("../presence/PresenceService");
const PacketRelay_1 = require("../presence/PacketRelay");
const ZoneOwnership_1 = require("../presence/ZoneOwnership");
const ZoneHandoff_1 = require("../presence/ZoneHandoff");
const crypto_1 = require("crypto");
const handlers_1 = require("./handlers");
const ProcSystem_1 = require("../combat/ProcSystem");
...
class NetworkServer {
    addToZonePlayerIndex(zoneId, characterId) {
        this.zoneRegistry.getOrCreate(zoneId).addPlayer(characterId);
    }
    removeFromZonePlayerIndex(characterId) {
        this.zoneRegistry.removePlayerFromAll(characterId);
    }
    movePlayerZoneIndex(characterId, newZoneId) {
        this.removeFromZonePlayerIndex(characterId);
        this.addToZonePlayerIndex(newZoneId, characterId);
    }
    constructor(httpServer, presenceOpts = { redis: null, isRedisConnected: () => false }) {
        this.tickRate = 30;
        this.movementThrottle = new MovementThrottle_1.MovementThrottle();
        this.zoneRegistry = new ZoneInstance_1.ZoneRegistry();
        this.aiTickBucket = 0;
        this.AI_TICK_STAGGER = 4;
        ...
        const shardId = process.env.SHARD_ID || `shard-${(0, crypto_1.randomUUID)()}`;
        this.presence = new PresenceService_1.PresenceService(shardId, presenceOpts.redis, presenceOpts.isRedisConnected);
        this.packetRelay = new PacketRelay_1.PacketRelay(shardId, presenceOpts.redis, presenceOpts.isRedisConnected, this.presence, (id, pkt) => {
            const sid = this.state.playerToSocket.get(id);
            if (sid)
                this.sendToSocket(sid, pkt);
        });
        this.zoneOwnership = new ZoneOwnership_1.ZoneOwnership(shardId, presenceOpts.redis, presenceOpts.isRedisConnected);
        this.zoneHandoff = new ZoneHandoff_1.ZoneHandoff(presenceOpts.redis, presenceOpts.isRedisConnected);
        console.log(`Shard ID: ${shardId}`);
        this.auth = AuthManager_1.AuthManager.getInstance();
        this.combat = new CombatSystem_1.CombatSystem({ getEntity: () => undefined, getEntitiesWithComponent: () => [], getAllEntities: () => [], addComponent: () => { }, removeComponent: () => { }, createEntity: () => ({ id: '', components: new Map() }), removeEntity: () => { } });
        this.ai = new AISystem_1.AISystem({ getEntity: () => undefined, getEntitiesWithComponent: () => [], getAllEntities: () => [], addComponent: () => { }, removeComponent: () => { }, createEntity: () => ({ id: '', components: new Map() }), removeEntity: () => { } });
        this.loot = new LootSystem_1.LootSystem({ getEntity: () => undefined, getEntitiesWithComponent: () => [], getAllEntities: () => [], addComponent: () => { }, removeComponent: () => { }, createEntity: () => ({ id: '', components: new Map() }), removeEntity: () => { } });
        this.playerSys = new PlayerSystem_1.PlayerSystem({ getEntity: () => undefined, getEntitiesWithComponent: () => [], getAllEntities: () => [], addComponent: () => { }, removeComponent: () => { }, createEntity: () => ({ id: '', components: new Map() }), removeEntity: () => { } });
        this.skillSys = new SkillSystem_1.SkillSystem();
        this.enmity = new EnmitySystem_1.EnmitySystem();
        this.tradeSys = new TradeSystem_1.TradeSystem();
        this.ai.enmitySys = this.enmity;
        this.spawnMgr = new SpawnManager_1.SpawnManager();
        this.summonMgr = new SummonManager_1.SummonManager();
        this.questSys = new QuestSystem_1.QuestSystem();
        this.craftSys = new CraftSystem_1.CraftSystem();
        this.cutsceneSys = new CutsceneSystem_1.CutsceneSystem();
        this.itemSys = new ItemSystem_1.ItemSystem();
        ...
        this.partySys = new PartySystem_1.PartySystem({
        ...
        this.handlers = (0, handlers_1.registerAllHandlers)();
        this.setupCallbacks();
        ...
        this.dummyMgr = new DummyManager_1.DummyManager({
            ...
            getItemDef: (id) => this.itemSys.getItemDefinition(id),
        });
        this.spatialMgr = new SpatialIndexManager_1.SpatialIndexManager({
            getPlayers: () => this.state.players,
            getSpawnManager: () => this.spawnMgr,
        });
        this.aoeZoneMgr = new AOEZoneManager_1.AOEZoneManager({
            getPlayers: () => this.state.players,
            getSpawnManager: () => this.spawnMgr,
        ...
        this.songMgr = new SongProximityManager_1.SongProximityManager({
            getPlayers: () => this.state.players,
            getSkillSystem: () => this.skillSys,
            getPlayerSys: () => this.playerSys,
        ...
        this.blockingMgr = new BlockingProximityManager_1.BlockingProximityManager({
            getPlayers: () => this.state.players,
            getPlayerSys: () => this.playerSys,
            queryPlayersNear: (x, z, r, zid) => this.queryPlayersNear(x, z, r, zid),
            sendToPlayer: (id, pkt) => this.sendToPlayer(id, pkt),
        ...
        this.knockbackMgr = new KnockbackManager_1.KnockbackManager({
            getSpawnManager: () => this.spawnMgr,
     ...
     */
    async usePacketRelay(subClient) {
        await this.packetRelay.start(subClient);
    }
    async stopPacketRelay() {
    ...
    findCharacterBySocket(socketId) {
    ...
    registerPlayerInZone(characterId, zoneId) {
    ...
    unregisterPlayerFromZone(characterId) {
    ...
    movePlayerToZone(characterId, newZoneId) {
    ...
    getZoneIdForCharacter(characterId) {
        return this.zoneRegistry.zoneOf(characterId);
    }
    forEachPlayerInZone(zoneId, cb) {
     ...
     */
    cleanupPlayerZoneResources(session) {
     ...
     * shard).
     */
    completeZoneHandoffDeparture(session) {
        const characterId = session.characterId;
    ...
    broadcastInZone(zoneId, packet, excludeCharacterId) {
    ...
    broadcastEntityEffects(session) {
        this.broadcastInZone(session.zoneId, {
    ...
    grantQuestKillCredit(killer, enemyType, zoneId) {
        ...
        if (party) {
            for (const member of party.members) {
                ...
                if (!memberSession)
                    continue;
                if (memberSession.zoneId !== zoneId)
                    continue;
        ...
        for (const recipient of creditRecipients) {
            ...
            if (progress.progressed.length === 0 && progress.completed.length === 0)
                continue;
    ...
    checkQuestCellEntry(session) {
        const hasExploreObjective = session.quests.some(q => q.status === shared_1.QuestStatus.IN_PROGRESS && q.objectives.some(o => (o.type === shared_1.QuestType.EXPLORE || o.type === shared_1.QuestType.ESCORT) && o.cell && o.currentCount < o.requiredCount));
        if (!hasExploreObjective)
            return;
        const zoneDef = (0, shared_1.getZoneDefinition)(session.zoneId);
        if (!zoneDef)
            return;
        const cell = (0, shared_1.worldToCell)(session.position.x, session.position.z, zoneDef.size);
        if (!cell)
            return;
        const label = (0, shared_1.cellLabel)(cell);
        if (session.lastQuestCell === label)
            return;
    ...
    startCutscene(session, cutsceneId) {
        const cs = this.cutsceneSys.getCutscene(cutsceneId);
        if (!cs)
            return false;
        if (session.inCutscene)
            return false;
        ...
        this.sendToPlayer(session.characterId, {
            type: shared_1.PacketType.CUTSCENE_START,
            timestamp: Date.now(),
            data: { cutsceneId, steps: cs.steps }
        });
        return true;
    }
    completeCutscene(session) {
        if (!session.inCutscene)
            return;
        ...
        this.sendToPlayer(session.characterId, {
            type: shared_1.PacketType.CUTSCENE_END,
            timestamp: Date.now(),
            data: {
     ...
     */
    handleEnemyLoot(killer, enemyType, enemyPosition) {
        const enemyDef = (0, shared_1.getEnemyDefinition)(enemyType);
        if (!enemyDef)
            return;
        ...
        switch (rule) {
                ...
                for (const d of drops) {
                    ...
                    for (const m of party.members) {
                        this.sendToPlayer(m.characterId, {
                            type: shared_1.PacketType.PARTY_LOOT_ROLL,
                            timestamp: Date.now(),
            ...
            case shared_1.LootRule.NEED_GREED: {
                for (const d of drops) {
    ...
    broadcastLootSpawn(bag) {
        this.broadcastInZone(bag.zoneId, {
            type: shared_1.PacketType.LOOT_SPAWN,
            timestamp: Date.now(),
            data: {
                lootId: bag.id,
    ...
    tickLootExpiry() {
        ...
        for (const bag of expiredBags) {
            this.broadcastInZone(bag.zoneId, {
                type: shared_1.PacketType.LOOT_DESPAWN,
                timestamp: Date.now(),
                data: { lootId: bag.id }
            });
        }
    ...
    handleEnemyKill(enemyId, killerId) {
    ...
    handleRevivePlayerBySession(caster, targetId) {
        ...
        if (Math.sqrt(dx * dx + dz * dz) > 10) {
            this.sendToPlayer(caster.characterId, {
                type: shared_1.PacketType.SKILL_USE,
                timestamp: Date.now(),
    ...
    applyPlayerDamage(target, damage, attackerId, damageType, isCritical, zoneId, attackerPosition) {
        if (damage <= 0)
            return { redirected: false, damageTaken: 0 };
        this.cancelRest(target);
        if (target.statusEffects) {
            const invIdx = target.statusEffects.findIndex(e => e.type === shared_1.StatusEffectType.INVISIBLE);
            if (invIdx !== -1) {
        ...
        for (let i = 0; i < target.statusEffects.length; i++) {
            ...
            if (negationIdx === -1 && e.type === shared_1.StatusEffectType.BUFF_DAMAGE_NEGATION && e.buffData?.damageNegationThreshold) {
                negationIdx = i;
            }
            else if (protectedIdx === -1 && e.type === shared_1.StatusEffectType.BUFF_BLOCKING_PROTECTED && e.buffData?.blockingProtectedBy) {
                protectedIdx = i;
            }
            else if (selfBlockIdx === -1 && e.type === shared_1.StatusEffectType.BUFF_BLOCKING_STANCE && e.buffData?.blockingStance) {
                selfBlockIdx = i;
            }
        }
        if (negationIdx !== -1) {
        ...
        const barrierType = damageType === 'physical' ? shared_1.StatusEffectType.BARRIER_PHYSICAL : shared_1.StatusEffectType.BARRIER_MAGICAL;
        for (let i = 0; i < target.statusEffects.length; i++) {
        ...
        for (let i = 0; i < target.statusEffects.length; i++) {
            const e = target.statusEffects[i];
            if (e.type === shared_1.StatusEffectType.BUFF_MANA_SHIELD && e.buffData?.manaShield) {
        ...
        if (protectedIdx !== -1) {
            ...
            if (blocker && !blocker.isDead && blocker.statusEffects) {
                for (let i = 0; i < blocker.statusEffects.length; i++) {
                    if (blocker.statusEffects[i].type === shared_1.StatusEffectType.BUFF_BLOCKING_STANCE) {
                        blockStance = true;
                        break;
                    }
                }
            ...
            if (blocker && !blocker.isDead && blockStance) {
                ...
                if (attackerId && this.spawnMgr.getEnemy(attackerId) && Math.random() < 0.5) {
                    blocker.statusEffects.push({
                        ...
                        type: shared_1.StatusEffectType.KNOCKDOWN,
                        sourceId: attackerId,
    ...
    _applyRawDamage(target, damage, attackerId, damageType, isCritical, zoneId) {
        ...
        const obelisk = (0, obeliskBuff_1.getObeliskBuffForWeapon)(this.itemSys, target.equipment?.weapon);
    ...
    applyRemoveResistBuffs(target, appliedEffect) {
        if (!appliedEffect.removeResistBuffs || appliedEffect.removeResistBuffs.length === 0)
            return;
    ...
    applySkillKnockback(target, attackerPos, distance) {
        if (distance <= 0)
            return;
        if (target.statusEffects.some(e => e.type === shared_1.StatusEffectType.STUN || e.type === shared_1.StatusEffectType.FREEZE))
            return;
        if (target.currentNpcId)
            return;
        ...
        target.statusEffects.push({
            ...
            type: shared_1.StatusEffectType.BUFF_GENERIC,
            sourceId: '',
            targetId: '',
            potency: 0,
            appliedAt: Date.now(),
    ...
    tryInterruptCast(target) {
        if (!target.activeCast)
            return;
        ...
        if (Math.random() < interruptChance) {
            ...
            this.sendToPlayer(target.characterId, {
                type: shared_1.PacketType.COOLDOWN_UPDATE,
                timestamp: Date.now(),
                data: { skillName, type: 'cast_cancel' }
            });
        }
    }
    damageEnemy(enemy, damage, attackerId) {
    ...
    getEnemyEffectiveDefense(enemy) {
        let defense = enemy.level * 0.5;
        const def = (0, shared_1.getEnemyDefinition)(enemy.enemyType);
        if (def)
            defense = def.defense || 0;
        for (const effect of enemy.statusEffects) {
            if (effect.type === shared_1.StatusEffectType.DEBUFF_DEFENSE_DOWN) {
                defense = Math.floor(defense * (1 - (effect.potency || 0)));
            }
        }
        return defense;
    }
    sendDamageDebug(session, result) {
        if (result.debugCalc) {
            this.sendToPlayer(session.characterId, {
    ...
    getTargetStatsForEntity(id) {
        ...
        if (player) {
            const eff = (0, shared_1.getEffectiveStats)(player.stats, player.statPoints, player.statusEffects || []);
            return {
                defense: eff.defense,
                magicDefense: Math.floor(eff.defense * 0.3),
                health: player.stats.health,
                level: player.stats.level,
                dodge: (0, shared_1.calculateDodge)(player.stats.level, (player.statPoints.AGI || 0) + ((player.baseStats || { AGI: 0 }).AGI || 0), eff.dodgeBonus),
    ...
    findClosestEntityToPosition(session, pos, radius) {
        let closest = null;
        ...
        for (const entry of nearPlayers) {
            if (entry.id === session.characterId)
                continue;
            const dx = entry.x - pos.x;
            const dz = entry.z - pos.z;
            const dist = Math.sqrt(dx * dx + dz * dz);
    ...
    findAllEntitiesInRadius(session, pos, radius) {
        const results = [];
        const nearEnemies = this.queryEnemiesNear(pos.x, pos.z, radius, session.zoneId);
        for (const entry of nearEnemies) {
            const dx = entry.x - pos.x;
    ...
    consumeDebuffsOnHit(targetSession) {
    ...
    shouldApplyDebuff(effect, targetId, casterId) {
        if (!effect.debuffCategory)
            return true;
        ...
        if (casterSession) {
            ...
            accuracy = (0, shared_1.computeDebuffAccuracy)(totalSPI, prof, effect.debuffCategory);
        }
        const resistPercent = this.getDebuffResist(targetId, effect.debuffCategory);
        const { applied, roll } = (0, shared_1.rollDebuffApplication)(accuracy, resistPercent);
    ...
    getDebuffResist(targetId, category) {
        const player = this.state.players.get(targetId);
        if (player) {
            if (!player.resistCache)
                player.resistCache = new Map();
            ...
            if (STA_DEBUFF_CATEGORIES.has(category)) {
                const gearKey = `${category}Resist`;
                const gearBonus = gc?.[gearKey] || 0;
                result = (0, shared_1.computeAilmentResist)(totalSTA, gearBonus);
            }
            else if (SPI_DEBUFF_CATEGORIES.has(category)) {
                const gearKey = `${category}Resist`;
                const gearBonus = gc?.[gearKey] || 0;
                result = (0, shared_1.computeDisorderResist)(totalSPI, gearBonus);
            }
        ...
        if (enemy) {
            return category === 'ailment' ? enemy.ailmentResist || 0 : enemy.disorderResist || 0;
        }
        return 0;
    }
    hasActiveDebuff(targetId, effectType, skillName) {
        const player = this.state.players.get(targetId);
        if (player) {
    ...
    processOnHitProcs(session, targetId, damageDealt, isPhysical) {
        if (!isPhysical || damageDealt <= 0)
            return;
        const { procs, enhancementLevel, enhancementElement } = (0, ProcSystem_1.collectProcs)(session, this.itemSys);
        for (const proc of procs) {
            if (proc.minLevel && enhancementLevel < proc.minLevel)
                continue;
            let chance = proc.baseChance + (proc.chancePerLevel || 0) * enhancementLevel;
            if (chance <= 0)
                continue;
            const resistCategory = (0, ProcSystem_1.getProcResistCategory)(proc);
            if (resistCategory) {
        ...
        if (enhancementElement === 'dark') {
            const recoilMultiplier = (0, ProcSystem_1.getDarkRecoilPercent)(enhancementLevel);
    ...
    applyProcEffect(proc, session, targetId, damageDealt) {
        if ((0, ProcSystem_1.isDrainLifeProc)(proc)) {
        ...
        const effect = (0, ProcSystem_1.buildProcStatusEffect)(proc, session.characterId, targetId);
        if (!effect)
            return;
        if (this.hasActiveDebuff(targetId, effect.type, effect.skillName))
            return;
        const enemy = this.spawnMgr.getEnemy(targetId);
        if (enemy) {
    ...
    processGloomRecoil(session) {
        const darkAuras = session.statusEffects?.filter(e => e.type === shared_1.StatusEffectType.WEAPON_AURA && e.buffData?.weaponAura?.element === 'dark') || [];
        if (darkAuras.length === 0)
            return;
        const darknessAdeptness = session.skillAdeptness?.['Darkness'] || 0;
        const recoilRate = (0, shared_1.getGloomRecoilRate)(darknessAdeptness);
        const effective = (0, shared_1.getEffectiveStats)(session.stats, session.statPoints, session.statusEffects || []);
    ...
    findGuardian(targetId) {
        ...
        if (target && (0, obeliskBuff_1.getObeliskBuffForWeapon)(this.itemSys, target.equipment?.weapon)) {
            return null;
        }
        for (const [, session] of this.state.players) {
            const guardianEffect = session.statusEffects?.find(e => e.type === shared_1.StatusEffectType.BUFF_DAMAGE_REDIRECT && e.buffData?.damageRedirectTargetId === targetId);
            if (guardianEffect)
                return session;
        }
        return null;
     ...
     */
    breakGuardianLinksFor(characterId) {
        const session = this.state.players.get(characterId);
        if (!session || !session.statusEffects)
            return;
        // Direction 1: this character is the PROTECTOR. Strip own redirect and the
        // protected target's BUFF_GUARDED marker.
        const redirectFx = session.statusEffects.find(e => e.type === shared_1.StatusEffectType.BUFF_DAMAGE_REDIRECT && e.buffData?.damageRedirectTargetId);
        if (redirectFx) {
            ...
            if (guarded) {
                guarded.statusEffects = guarded.statusEffects.filter(e => e.type !== shared_1.StatusEffectType.BUFF_GUARDED);
        ...
        const guardedFx = session.statusEffects.find(e => e.type === shared_1.StatusEffectType.BUFF_GUARDED);
        if (guardedFx) {
            session.statusEffects = session.statusEffects.filter(e => e !== guardedFx);
            this.syncGuardianRemoval(characterId);
            for (const [, protector] of this.state.players) {
                const fx = protector.statusEffects?.find(e => e.type === shared_1.StatusEffectType.BUFF_DAMAGE_REDIRECT && e.buffData?.damageRedirectTargetId === characterId);
                if (fx) {
    ...
    syncGuardianRemoval(characterId) {
    ...
    setupCallbacks() {
        this.combat.onDeath((targetId, killerId) => {
            ...
            enemy.state = 'dead';
            enemy.deathTime = Date.now();
            this.broadcastInZone(this.findZoneOfEntity(targetId), {
                type: shared_1.PacketType.DEATH,
                timestamp: Date.now(),
        ...
        this.ai.onEnemyAttack((enemyId, targetId, _rawDamage) => {
            ...
            if (!target)
                return;
            if (target.invulnerableUntil > Date.now())
                return;
            const enemy = this.spawnMgr.getEnemy(enemyId);
    ...
    setupPlayerCallbacks() {
        this.playerSys.onLevelUp((characterId, newLevel) => {
    ...
    async handlePacket(socket, packet) {
        try {
            if (packet.type === shared_1.PacketType.HEARTBEAT) {
                this.sendToSocket(socket.id, { type: shared_1.PacketType.HEARTBEAT, timestamp: Date.now(), data: {} });
                return;
            }
            if (packet.type === shared_1.PacketType.PLAYER_MOVE) {
    ...
    sendZoneState(socket, zoneId, includePlayerId) {
        ...
        const npcs = (0, shared_1.getNPCsInZone)(zoneId);
        ...
        this.state.players.forEach(player => {
            if (player.zoneId !== zoneId)
                return;
            if (player.characterId === this.findCharacterBySocket(socket.id) && player.characterId !== includePlayerId)
                return;
            otherPlayers.push({
    ...
    removeBlockingProtectedBuffs(blockerId) {
    ...
    executeAOESkillInternal(session, skillName, aoePosition) {
        const characterId = session.characterId;
        const skill = this.skillSys.findSkillDefinition(skillName);
        const aoeRadius = skill?.aoeRadius || shared_1.DEFAULT_AOE_RADIUS;
        ...
        if (result.summonObject) {
            const summon = this.summonMgr.spawnSummon(characterId, session.characterName, session.zoneId, result.summonObject, aoePosition, session.rotation?.y || 0, result.element);
            if (summon) {
                this.broadcastInZone(session.zoneId, {
                    type: shared_1.PacketType.ENTITY_SPAWN,
                    timestamp: Date.now(),
        ...
        if (result.banishObject) {
            const banishRadius = result.banishRadius || shared_1.BANISH_RADIUS;
            ...
            for (const target of targets) {
                ...
                if (enemy && enemy.state !== 'dead') {
                    ...
                    for (const effect of result.statusEffects) {
                        if (!this.shouldApplyDebuff(effect, target.id, session.characterId))
                            continue;
                        if (this.hasActiveDebuff(target.id, effect.type, effect.skillName))
                            continue;
                        const cloned = { ...effect, targetId: target.id };
                        enemy.statusEffects.push(cloned);
                        this.applyRemoveResistBuffs(enemy, cloned);
                        anyApplied = true;
                        if (effect.potency > maxPotency)
                            maxPotency = effect.potency;
                    }
    ...
    executeConeSkillInternal(session, skillName, targetId) {
        const characterId = session.characterId;
        const skill = this.skillSys.findSkillDefinition(skillName);
        if (!skill)
            return;
        ...
        for (const entry of candidates) {
            ...
            if (this.state.players.has(entry.id)) {
                if (this.isPartyMember(characterId, entry.id))
                    continue;
            ...
            for (const dir of bladeDirs) {
                ...
                if (result.elementalDamage)
                    allElemental.push(...result.elementalDamage);
                if (result.physicalDamage)
                    totalPhysical += result.physicalDamage;
                if (result.physicalElementalDamage)
                    allPhysicalElemental.push(...result.physicalElementalDamage);
            }
            if (totalDamage === 0)
                continue;
            if (enemy) {
                const { died: mainDied } = this.damageEnemy(enemy, totalDamage, characterId);
                for (const el of allElemental)
                ...
                if (totalPhysical && enemy.health > 0) {
                    ...
                    for (const el of allPhysicalElemental)
                        this.damageEnemy(enemy, el.damage, characterId);
                }
                this.broadcastInZone(session.zoneId, {
                    type: shared_1.PacketType.DAMAGE,
                ...
                if (died) {
                    ...
                    this.broadcastInZone(session.zoneId, { type: shared_1.PacketType.ENTITY_DESPAWN, timestamp: Date.now(), data: { entityId: entry.id } });
                }
                else {
                    this.broadcastInZone(session.zoneId, {
                        type: shared_1.PacketType.STATS_UPDATE,
    ...
    applyDevotionRefund(session, skillName, devotionFx) {
    ...
    handleAOECastCompletion(session, castResult) {
    ...
    applyAOEDamageToTargets(session, skillName, aoePosition, aoeRadius, primaryResult) {
        const characterId = session.characterId;
        const targets = this.findAllEntitiesInRadius(session, aoePosition, aoeRadius);
        const skill = this.skillSys.findSkillDefinition(skillName);
        const batchEvents = [];
        for (const target of targets) {
            if (target.id !== characterId && this.state.players.has(target.id)) {
                if (this.isPartyMember(characterId, target.id))
                    continue;
                if (!(0, shared_1.isZonePvpEnabled)(session.zoneId))
                    continue;
            }
            ...
            if (enemy) {
                ...
                if (targetResult.statusEffects && targetResult.statusEffects.length > 0) {
                    ...
                    for (const effect of targetResult.statusEffects) {
                        if (!this.shouldApplyDebuff(effect, target.id, characterId))
                            continue;
                        if (this.hasActiveDebuff(target.id, effect.type, effect.skillName))
                            continue;
                        ...
                        anyApplied = true;
                        if (applied.potency > maxPotency)
                            maxPotency = applied.potency;
                    }
                    if (anyApplied) {
                ...
                if (playerTarget && target.id !== characterId) {
                    const coneTotalDmg = targetResult.damage
                        + (targetResult.elementalDamage?.reduce((s, e) => s + e.damage, 0) || 0)
                        + (targetResult.physicalDamage || 0)
                        + (targetResult.physicalElementalDamage?.reduce((s, e) => s + e.damage, 0) || 0);
        ...
        if (batchEvents.length > 0) {
            this.broadcastInZone(session.zoneId, {
                type: shared_1.PacketType.BATCH_COMBAT,
                timestamp: Date.now(),
                data: { events: batchEvents }
            });
        }
    }
    applySingleTargetSkillDamage(session, skillName, targetId, result) {
        const characterId = session.characterId;
        ...
        if (hits && hits.length > 1) {
            ...
            if (playerTarget && targetId !== characterId) {
                for (const hit of hits) {
                    const hitTotal = hit.damage + (hit.elementalDamage?.reduce((s, e) => s + e.damage, 0) || 0);
        ...
        const playerTarget = this.state.players.get(targetId);
        if (playerTarget && targetId !== characterId) {
            const totalPvpDmg = (result.damage || 0) + (result.elementalDamage?.reduce((s, e) => s + e.damage, 0) || 0);
    ...
    tickSongProximity(now) {
        this.songMgr.tick(now);
    }
    applyRedSongDamage(caster, skill, songRadius) {
        if (!caster.position)
            return;
        ...
        for (const [targetId, target] of this.state.players) {
            ...
            if (!target.position || !caster.position)
                continue;
            if (this.isPartyMember(caster.characterId, targetId))
                continue;
    ...
    applyAOEDebuffsOnEnter(caster, skillName, targetId) {
        const skill = this.skillSys.findSkillDefinition(skillName);
        ...
        if (enemy && enemy.state !== 'dead') {
            ...
            for (const effect of debuffEffects) {
                if (enemy.statusEffects.some(e => e.type === effect.type && e.skillName === skillName))
                    continue;
        ...
        if (player && targetId !== caster.characterId) {
            let changed = false;
            for (const effect of debuffEffects) {
                if (!this.shouldApplyDebuff(effect, targetId, caster.characterId))
                    continue;
                if (player.statusEffects.some(e => e.type === effect.type && e.skillName === skillName))
                    continue;
                const applied = { ...effect, targetId };
    ...
    tickPlayerUpdates(now) {
        this.state.players.forEach(session => {
            if (session.isDead)
                return;
            if (session.resistCache)
                session.resistCache.clear();
            ...
            if (castResult?.completed) {
                ...
                if (completedSkillDef?.aoeTargetMode === shared_1.AOETargetMode.CONE) {
                    const devotionFx = session.statusEffects?.find(e => e.buffData?.devotionLink);
                    if (devotionFx) {
                        this.applyDevotionRefund(session, castResult.skillName, devotionFx);
                    }
                ...
                if (result.guardianApplied) {
                    ...
                    if (guarded) {
                        ...
                        guarded.statusEffects.push({
                            ...
                            type: shared_1.StatusEffectType.BUFF_GUARDED,
                ...
                if (result.soulSwap && castResult.targetId) {
                    const target = this.state.players.get(castResult.targetId);
                    if (target) {
                        const casterBuffs = session.statusEffects.filter(e => !e.type.startsWith('DEBUFF_') && e.type !== shared_1.StatusEffectType.CURSE);
                        const targetBuffs = target.statusEffects.filter(e => !e.type.startsWith('DEBUFF_') && e.type !== shared_1.StatusEffectType.CURSE);
                        session.statusEffects = [...session.statusEffects.filter(e => e.type.startsWith('DEBUFF_') || e.type === shared_1.StatusEffectType.CURSE), ...targetBuffs.map(b => ({ ...b, sourceId: session.characterId, targetId: session.characterId }))];
                        target.statusEffects = [...target.statusEffects.filter(e => e.type.startsWith('DEBUFF_') || e.type === shared_1.StatusEffectType.CURSE), ...casterBuffs.map(b => ({ ...b, sourceId: target.characterId, targetId: target.characterId }))];
                        this.playerSys.recalcStats(session);
                ...
                if (castSkill) {
                    const castTargetType = shared_1.SKILL_TARGET_RULES[castResult.skillName];
                    if (castSkill.duration > 0 && castResult.targetId && castResult.targetId !== session.characterId && !castSkill.debuffEffectTable && !castSkill.isDebuff) {
                        const shouldApply = !castTargetType
                            || castTargetType === shared_1.SkillTargetType.SELF_OR_TARGET
                            || castTargetType === shared_1.SkillTargetType.PARTY
                            || castTargetType === shared_1.SkillTargetType.OTHER_ONLY;
                        if (shouldApply) {
                    ...
                    if (castTargetType === shared_1.SkillTargetType.PARTY) {
                        ...
                        for (const memberId of partyMembers) {
                            ...
                            if (!memberSession || memberSession.isDead || memberSession.stats.health <= 0)
                                continue;
                            if (memberSession.zoneId !== session.zoneId)
                                continue;
                ...
                if (result.statusEffects && result.statusEffects.length > 0 && castResult.targetId) {
                    ...
                    if (debuffTarget) {
                        for (const effect of result.statusEffects) {
                            if (!this.shouldApplyDebuff(effect, castResult.targetId, session.characterId))
                                continue;
                            if (this.hasActiveDebuff(castResult.targetId, effect.type, effect.skillName))
                                continue;
                        ...
                        if (debuffEnemy && debuffEnemy.state !== 'dead') {
                            ...
                            const debuffEnemyDef = (0, shared_1.getEnemyDefinition)(debuffEnemy.enemyType);
                ...
                if (!result.damage && (!result.statusEffects || result.statusEffects.length === 0) && !result.provoked && !result.healing && castResult.targetId) {
                    ...
                    if (fallbackSkillDef) {
                        ...
                        if (fst === shared_1.SkillType.DEBUFF || fst === shared_1.SkillType.FEAR || fst === shared_1.SkillType.DISPEL) {
            ...
            if (session.stats.health > 0 && now - session.lastRegenTick >= shared_1.REGEN_CONFIG.TICK_INTERVAL_MS) {
                let lpGain;
                let mpGain;
                if (session.isResting) {
                    const restTicksElapsed = Math.floor((now - session.restStartedAt) / shared_1.REGEN_CONFIG.TICK_INTERVAL_MS);
                    let rampUp = 1;
                    if (restTicksElapsed <= 1)
                        rampUp = shared_1.REST_REGEN_CONFIG.RAMP_TICK_0;
                    else if (restTicksElapsed === 2)
                        rampUp = shared_1.REST_REGEN_CONFIG.RAMP_TICK_1;
                    const lpRegen = Math.floor(session.stats.maxHealth / shared_1.REST_REGEN_CONFIG.HP_DIVISOR)
                        + Math.floor(session.statPoints.STA / shared_1.REST_REGEN_CONFIG.HP_STA_DIVISOR);
                    const mpRegen = Math.floor(session.stats.maxMana / shared_1.REST_REGEN_CONFIG.MP_DIVISOR)
                        + Math.floor(session.statPoints.SPI / shared_1.REST_REGEN_CONFIG.MP_SPI_DIVISOR);
                    lpGain = Math.floor(lpRegen * rampUp);
                    mpGain = Math.floor(mpRegen * rampUp);
                }
                else {
                    const inCombat = now - session.lastAttackTime < shared_1.REGEN_CONFIG.OUT_OF_COMBAT_DELAY_MS;
                    const lpRegen = Math.floor(session.stats.maxHealth / shared_1.REGEN_CONFIG.LP_DIVISOR)
                        + Math.floor(session.statPoints.STA / shared_1.REGEN_CONFIG.LP_STA_DIVISOR);
                    const mpRegen = Math.floor(session.stats.maxMana / shared_1.REGEN_CONFIG.MP_DIVISOR)
                        + Math.floor(session.statPoints.SPI / shared_1.REGEN_CONFIG.MP_SPI_DIVISOR);
                    lpGain = inCombat ? Math.floor(lpRegen * shared_1.REGEN_CONFIG.IN_COMBAT_LP_MULTIPLIER) : lpRegen;
                    mpGain = inCombat ? Math.floor(mpRegen * shared_1.REGEN_CONFIG.IN_COMBAT_MP_MULTIPLIER) : mpRegen;
                }
                let changed = false;
            ...
            const blockStanceEffect = session.statusEffects?.find(e => e.type === shared_1.StatusEffectType.BUFF_BLOCKING_STANCE && e.buffData?.blockingStance);
            if (blockStanceEffect) {
                const lastDrain = blockStanceEffect.lastMpDrainAt || blockStanceEffect.appliedAt;
                if (now - lastDrain >= 2000) {
                    blockStanceEffect.lastMpDrainAt = now;
            ...
            const guardianRedirectFx = session.statusEffects?.find(e => e.type === shared_1.StatusEffectType.BUFF_DAMAGE_REDIRECT && e.buffData?.damageRedirectTargetId);
            if (guardianRedirectFx && session.position) {
                ...
                if (guardedPlayer && guardedPlayer.position) {
                    ...
                    if (gdist > 20) {
                        ...
                        this.broadcastEntityEffects(session);
                        guardedPlayer.statusEffects = guardedPlayer.statusEffects.filter(e => e.type !== shared_1.StatusEffectType.BUFF_GUARDED);
    ...
    findClosestEnemyToPosition(zoneId, pos, range) {
    ...
    tickAI(now) {
        ...
        const reusableZonePlayers = new Map();
        for (const zoneId of this.spawnMgr.getZoneIds()) {
            ...
            if (zonePlayerIds) {
                for (const cid of zonePlayerIds) {
                    ...
                    if (!s || s.isDead || s.invulnerableUntil > now)
                        continue;
                    if (s.currentNpcId)
                        continue;
                    if (s.statusEffects?.some(e => e.type === shared_1.StatusEffectType.INVISIBLE))
                        continue;
                    if (s.statusEffects?.some(e => e.buffData?.misdirection))
                        continue;
                    zonePlayers.set(cid, { position: s.position, characterId: cid });
                }
            }
            const zoneSummons = new Map();
            ...
            this.ai.updateDecisions(this.spawnMgr.getEnemiesInZone(zoneId), zonePlayers, zoneSummons, this.AI_TICK_STAGGER / this.tickRate, this.aiTickBucket, this.AI_TICK_STAGGER);
            this.ai.updateMovement(this.spawnMgr.getEnemiesInZone(zoneId), zonePlayers, zoneSummons, 1 / this.tickRate);
        }
    }
    ...
    broadcastEntityStates() {
        ...
        for (const zoneId of this.spawnMgr.getZoneIds()) {
            ...
            if (!zoneEnemies)
                continue;
            const zonePlayerIds = this.zoneRegistry.get(zoneId)?.players;
            if (!zonePlayerIds || zonePlayerIds.size === 0)
                continue;
    ...
    async saveAllCharacters() {
        ...
        for (let i = 0; i < sessions.length; i += BATCH_SIZE) {
            ...
            await Promise.all(batch.map(session => this.auth.saveCharacter(session.characterId, {
                level: session.stats.level,
                experience: session.stats.experience,
                position: session.position,
                zoneId: session.zoneId,
    ...
    getLastMoveBroadcast(characterId) {
        return this.movementThrottle.get(characterId);
    }
    setLastMoveBroadcast(characterId, time) {
        this.movementThrottle.set(characterId, time);
    }
    clearMovementThrottle(characterId) {
        this.movementThrottle.clear(characterId);
    }
}
exports.NetworkServer = NetworkServer;
NetworkServer.INTEREST_RADIUS = 50;
NetworkServer.INTEREST_RADIUS_SQ = 50 * 50;
