...
function registerHandlers(registry) {
    registry.set(shared_1.PacketType.CRAFT_REQUEST, handleCraftRequest);
}
/**
 ...
 */
function handleCraftRequest(ctx, socket, data) {
    ...
    if (!recipeId || !npcId) {
        ctx.sendToPlayer(characterId, { type: shared_1.PacketType.ERROR, timestamp: Date.now(), data: { message: 'Invalid craft request.' } });
        return;
    }
    const npc = shared_1.NPC_DATABASE[npcId];
    if (!npc) {
        ctx.sendToPlayer(characterId, { type: shared_1.PacketType.ERROR, timestamp: Date.now(), data: { message: 'Unknown NPC.' } });
        return;
    }
    // Range check
