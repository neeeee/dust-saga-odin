...
function registerHandlers(registry) {
    registry.set(shared_1.PacketType.WEAPON_ENHANCE, handleWeaponEnhance);
}
function handleWeaponEnhance(ctx, socket, data) {
    ...
    if (!weaponSlot?.slotIndex && weaponSlot?.slotIndex !== 0) {
        ctx.sendToPlayer(characterId, { type: shared_1.PacketType.NOTIFICATION, timestamp: Date.now(), data: { message: 'No weapon selected.', type: 'error' } });
        return;
    }
    const weaponItem = session.inventory[weaponSlot.slotIndex];
    if (!weaponItem) {
        ctx.sendToPlayer(characterId, { type: shared_1.PacketType.NOTIFICATION, timestamp: Date.now(), data: { message: 'Weapon not found in inventory.', type: 'error' } });
        return;
    }
    const weaponDef = ctx.itemSys.getItemDefinition(weaponItem.itemId);
    if (!weaponDef) {
        ctx.sendToPlayer(characterId, { type: shared_1.PacketType.NOTIFICATION, timestamp: Date.now(), data: { message: 'Invalid weapon.', type: 'error' } });
        return;
    }
    const enhancableTypes = ['weapon', 'armor', 'helmet', 'boots', 'gloves', 'legs', 'shield'];
    if (!enhancableTypes.includes(weaponDef.type) && !weaponDef.equipmentSlot) {
        ctx.sendToPlayer(characterId, { type: shared_1.PacketType.NOTIFICATION, timestamp: Date.now(), data: { message: 'This item cannot be enhanced.', type: 'error' } });
        return;
    }
    const currentLevel = weaponItem.enhancementLevel || 0;
    if (currentLevel >= 10) {
        ctx.sendToPlayer(characterId, { type: shared_1.PacketType.NOTIFICATION, timestamp: Date.now(), data: { message: 'Enhancement level is already at maximum.', type: 'error' } });
        return;
    }
    const GEM_ELEMENT_MAP = {
    ...
    let element = weaponItem.enhancementElement || null;
    const materialSlotArr = materialSlots || [];
    const consumedSlots = [];
    for (const mat of materialSlotArr) {
        if (!mat?.slotIndex && mat?.slotIndex !== 0)
            continue;
        ...
        if (gemElement) {
            if (element === null) {
                ...
                ctx.sendToPlayer(characterId, { type: shared_1.PacketType.NOTIFICATION, timestamp: Date.now(), data: { message: 'Cannot mix different element gems.', type: 'error' } });
                return;
            }
    ...
    if (!element) {
        ctx.sendToPlayer(characterId, { type: shared_1.PacketType.NOTIFICATION, timestamp: Date.now(), data: { message: 'No element gem provided.', type: 'error' } });
        return;
    }
    const ENHANCE_FAILURE_CHANCE = [0, 0, 5, 15, 25, 35, 50, 65, 80, 90];
    ...
    if (Math.random() * 100 < failChance) {
        ...
        ctx.sendToPlayer(characterId, { type: shared_1.PacketType.NOTIFICATION, timestamp: Date.now(), data: { message: `Enhancement failed! Level remains at +${currentLevel}.`, type: 'error' } });
        ctx.sendToPlayer(characterId, {
            type: shared_1.PacketType.ENHANCEMENT_RESULT,
            timestamp: Date.now(),
    ...
    weaponItem.enhancementElement = element;
    const sortedSlots = [...consumedSlots].sort((a, b) => b - a);
    for (const slotIdx of sortedSlots) {
        const matItem = session.inventory[slotIdx];
        if (matItem) {
