...
function registerHandlers(registry) {
    registry.set(shared_1.PacketType.PLAYER_MOVE, handlePlayerMove);
}
const MOVE_BROADCAST_INTERVAL = 50;
function handlePlayerMove(ctx, socket, data) {
    ...
    if (session.currentNpcId)
        return;
    if (!shared_1.Validator.validatePosition(data.position))
        return;
    const prevPos = session.position;
    const moved = prevPos && (Math.abs(data.position.x - prevPos.x) > 0.01 ||
    ...
    if (moved) {
        ...
        const blockingEffect = session.statusEffects?.find((e) => e.type === shared_1.StatusEffectType.BUFF_BLOCKING_STANCE && e.buffData?.blockingStance && !e.buffData?.defensiveMarch);
        if (blockingEffect) {
        ...
        const invisIdx = session.statusEffects?.findIndex((e) => e.type === shared_1.StatusEffectType.INVISIBLE && e.buffData?.invisible?.stationaryOnly);
    ...
    if (now - last >= MOVE_BROADCAST_INTERVAL) {
        ...
        const isInvis = session.statusEffects?.some((e) => e.type === shared_1.StatusEffectType.INVISIBLE) || false;
