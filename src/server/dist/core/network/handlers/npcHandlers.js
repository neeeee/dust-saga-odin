...
function registerHandlers(registry) {
    registry.set(shared_1.PacketType.NPC_INTERACT, handleNPCInteract);
    registry.set(shared_1.PacketType.NPC_SHOP_BUY, handleShopBuy);
    registry.set(shared_1.PacketType.NPC_DIALOG_CLOSE, handleNpcDialogClose);
}
function handleNPCInteract(ctx, socket, data) {
    ...
    if (!session)
        return;
    const npc = shared_1.NPC_DATABASE[data.npcId];
    if (!npc)
        return;
    ...
    ctx.sendToPlayer(characterId, {
        type: shared_1.PacketType.NPC_DIALOG,
        timestamp: Date.now(),
        data: {
            npcId: data.npcId,
            npcName: npc.name,
    ...
    if (data.dialogId === 'join_nation') {
        ...
        if (nationOption?.actionData?.nation) {
            session.nation = nationOption.actionData.nation;
            const nationInfo = shared_1.NATION_ZONE_MAP[session.nation];
            if (nationInfo) {
                const nationZoneDef = (0, shared_1.getZoneDefinition)(nationInfo.zoneId);
...
function handleNpcDialogClose(ctx, socket, _data) {
    const characterId = ctx.findCharacterBySocket(socket.id);
    if (!characterId)
        return;
    const session = ctx.state.players.get(characterId);
...
function handleShopBuy(ctx, socket, data) {
    const characterId = ctx.findCharacterBySocket(socket.id);
    if (!characterId)
        return;
    const session = ctx.state.players.get(characterId);
    ...
    if (cost > 0 && session.gold < cost) {
        ctx.sendToPlayer(characterId, { type: shared_1.PacketType.NOTIFICATION, timestamp: Date.now(), data: { message: `Not enough gold. Need ${cost}g, have ${session.gold}g.`, type: 'error' } });
        return;
    }
 ...
 */
function handleAdvancementAction(ctx, characterId, session, npc, dialogId) {
    ...
    if (step === 'strip') {
        ...
        if (param)
            serveDialog(ctx, characterId, npc, param);
        return;
    }
    if (step === 'reset') {
    ...
    if (step === 'advance') {
        ...
        if (advanced) {
            ...
            ctx.broadcastInZone(session.zoneId, {
                type: shared_1.PacketType.FLOATING_TEXT,
                timestamp: Date.now(),
...
function serveDialog(ctx, characterId, npc, dialogId) {
    const dialog = npc.dialogs?.find((d) => d.id === dialogId) || npc.dialogs?.[0];
    ctx.sendToPlayer(characterId, {
        type: shared_1.PacketType.NPC_DIALOG,
        timestamp: Date.now(),
        data: {
