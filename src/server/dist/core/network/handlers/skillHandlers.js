...
function checkDevotionMana(ctx, session, mpCost) {
    if (session.stats.mana >= mpCost)
        return true;
...
function registerHandlers(registry) {
    registry.set(shared_1.PacketType.SKILL_USE, handleSkillUse);
    registry.set(shared_1.PacketType.SKILL_CANCEL, handleSkillCancel);
}
function handleSkillCancel(ctx, socket, _data) {
    const characterId = ctx.findCharacterBySocket(socket.id);
    if (!characterId)
        return;
    const session = ctx.state.players.get(characterId);
    if (!session || !session.activeCast)
        return;
...
function handleSkillUse(ctx, socket, data) {
    const characterId = ctx.findCharacterBySocket(socket.id);
    if (!characterId)
        return;
    const session = ctx.state.players.get(characterId);
    ...
    if (aoePosition && shared_1.GROUND_TARGETED_AOE_SKILLS.has(skillName)) {
        handleGroundAOESkillUse(ctx, session, skillName, aoePosition);
        return;
    }
    const earlySkillDef = ctx.skillSys.findSkillDefinition(skillName);
    if (earlySkillDef?.aoeTargetMode === shared_1.AOETargetMode.CONE) {
        handleConeSkillUse(ctx, session, skillName, targetId);
        return;
    }
    if (targetId && targetId !== characterId && ctx.state.players.has(targetId) && !(0, shared_1.isZonePvpEnabled)(session.zoneId)) {
        const skillType = ctx.skillSys.getSkillType(skillName);
        if (skillType !== undefined && !(0, shared_1.isBeneficialSkillType)(skillType)) {
            ctx.sendToPlayer(characterId, {
    ...
    if (earlySkillDef?.buffEffectTable?.damageRedirect && targetId && targetId !== characterId) {
        ...
        if (targetSession && (0, obeliskBuff_1.getObeliskBuffForWeapon)(ctx.itemSys, targetSession.equipment?.weapon)) {
            ctx.sendToPlayer(characterId, {
                type: shared_1.PacketType.SKILL_USE,
                timestamp: Date.now(),
    ...
    if (targetId && targetId !== characterId && !aoePosition) {
        ...
        const skillType = earlySkillDef.skillType;
        const isBeneficial = (0, shared_1.isBeneficialSkillType)(skillType) ||
            skillType === shared_1.SkillType.HEAL || skillType === shared_1.SkillType.PARTY_HEAL ||
            skillType === shared_1.SkillType.BUFF || skillType === shared_1.SkillType.HP_BUFF ||
            skillType === shared_1.SkillType.MP_RESTORE || skillType === shared_1.SkillType.REVIVE;
        ...
        if (!(isBeneficial && isTargetEnemy) && tp && earlySkillDef) {
            ...
            let maxRange = earlySkillDef.range || 0;
            if (maxRange === 0) {
                const weapon = session.equipment?.weapon;
                const wdef = weapon ? ctx.itemSys.getItemDefinition(weapon.itemId) : null;
                const isRangedWeapon = wdef?.weaponType && shared_1.RANGED_WEAPON_TYPES.has(wdef.weaponType);
                const isMagical = earlySkillDef.damageType === 'magical' ||
                    skillType === shared_1.SkillType.DAMAGE_MAGICAL;
                maxRange = (isRangedWeapon || isMagical) ? 20 : (isBeneficial ? 25 : 4);
            }
            if (dist > maxRange) {
                ctx.sendToPlayer(characterId, {
                    type: shared_1.PacketType.SKILL_USE, timestamp: Date.now(),
    ...
    if (earlySkillDef?.lineOfSightRequired && targetId) {
        ...
        if (targetSession || targetEnemy) {
            ...
            if (targetPos && !(0, LineOfSight_1.hasLineOfSight)(session.zoneId, { x: session.position.x, z: session.position.z }, { x: targetPos.x, z: targetPos.z })) {
                ctx.sendToPlayer(characterId, {
                    type: shared_1.PacketType.SKILL_USE,
                    timestamp: Date.now(),
    ...
    const skillTargetRule = shared_1.SKILL_TARGET_RULES[skillName];
    const effectiveTargetType = earlySkillDef ? (0, shared_1.getSkillTargetType)(earlySkillDef) : undefined;
    const resolvedTargetType = effectiveTargetType || skillTargetRule;
    const isHarmfulSkill = !resolvedTargetType || resolvedTargetType === shared_1.SkillTargetType.OTHER_ONLY;
    const isBuffSkill = earlySkillDef?.isBuff || !!earlySkillDef?.buffEffectTable || earlySkillDef?.isRevive || earlySkillDef?.skillType === shared_1.SkillType.REVIVE || earlySkillDef?.skillType === shared_1.SkillType.HEAL || earlySkillDef?.skillType === shared_1.SkillType.PARTY_HEAL || !!earlySkillDef?.healing;
    ...
    if (!started)
        return;
    const invIdx = session.statusEffects?.findIndex(e => e.type === shared_1.StatusEffectType.INVISIBLE);
    ...
    let firstTargetId = targetId || null;
    ...
    if (firstTargetId && firstTargetId !== characterId) {
        const st = earlySkillDef.skillType;
        const isBeneficial = (0, shared_1.isBeneficialSkillType)(st) ||
            st === shared_1.SkillType.HEAL || st === shared_1.SkillType.PARTY_HEAL ||
            st === shared_1.SkillType.BUFF || st === shared_1.SkillType.HP_BUFF ||
            st === shared_1.SkillType.MP_RESTORE || st === shared_1.SkillType.REVIVE;
        if (isBeneficial) {
    ...
    if (skill?.isAOE && aoePosition) {
        const firstTarget = ctx.findClosestEntityToPosition(session, aoePosition, aoeRadius);
    ...
    if (result.summonObject) {
        ...
        const summon = ctx.summonMgr.spawnSummon(characterId, session.characterName, session.zoneId, result.summonObject, summonPos, session.rotation?.y || 0, result.element);
        if (summon) {
            ctx.broadcastInZone(session.zoneId, {
    ...
    if (result.statusEffects && result.statusEffects.length > 0) {
        if (aoePosition) {
            ...
            for (const target of targets) {
                ...
                if (enemy && enemy.state !== 'dead') {
                    ...
                    for (const effect of result.statusEffects) {
                        if (!ctx.shouldApplyDebuff(effect, target.id, characterId))
                            continue;
                        if (ctx.hasActiveDebuff(target.id, effect.type, effect.skillName))
                            continue;
            ...
            if (enemy && enemy.state !== 'dead') {
                ...
                let maxPotency = 0;
                for (const effect of result.statusEffects) {
                    if (!ctx.shouldApplyDebuff(effect, firstTargetId, characterId))
                        continue;
                    if (ctx.hasActiveDebuff(firstTargetId, effect.type, effect.skillName))
                        continue;
    ...
    if (result.healing && result.healing > 0) {
        ...
        ctx.spawnMgr.forEnemiesInZone(session.zoneId, enemy => {
            if (enemy.state === 'dead' || !enemy.enmityTable)
                return;
...
function handleGroundAOESkillUse(ctx, session, skillName, aoePosition) {
...
function handleConeSkillUse(ctx, session, skillName, targetId) {
    const characterId = session.characterId;
