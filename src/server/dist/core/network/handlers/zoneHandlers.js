...
function registerHandlers(registry) {
    registry.set(shared_1.PacketType.ENTER_ZONE, handleEnterZone);
}
async function handleEnterZone(ctx, socket, data) {
    ...
    if (!session)
        return;
    const targetZone = (0, shared_1.getZoneDefinition)(data.zoneId);
    if (!targetZone)
        return;
    ...
    if (targetShard !== ctx.zoneOwnership.getShardId()) {
        ...
        if (ok) {
            ...
            ctx.sendToPlayer(characterId, {
                type: shared_1.PacketType.ZONE_HANDOFF,
                timestamp: Date.now(),
                data: {
                    targetShard,
    ...
    if (targetZone.type === shared_1.ZoneType.SAFE || targetZone.type === shared_1.ZoneType.NATION) {
    ...
    ctx.sendToPlayer(characterId, {
        type: shared_1.PacketType.ENTER_ZONE,
        timestamp: Date.now(),
