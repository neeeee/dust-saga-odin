...
exports.DummyManager = void 0;
const shared_1 = require("@dust-saga/shared");
class DummyManager {
    constructor(deps) {
        this.deps = deps;
        this.dummyMeta = new Map();
        this.counter = 0;
    }
    gm(characterId, message) {
    ...
    spawnDummy(session) {
        ...
        const dummyJob = shared_1.JobId.WARRIOR;
        ...
        const dummySession = {
            ...
            race: 'human',
            jobId: dummyJob,
            baseClass: shared_1.BaseClass.WARRIOR,
            ...
            skillProficiencies: (0, shared_1.createDefaultSkillProficiencies)(),
            skillAdeptness: (0, shared_1.createDefaultSkillAdeptness)((0, shared_1.getDesignJobId)(dummyJob)),
            ...
            inventory: [],
            gold: 0,
            equipment: (0, shared_1.normalizeEquipment)(null),
            quests: [],
        ...
        this.deps.broadcastInZone(session.zoneId, {
            ...
            data: {
                ...
                data: {
                    ...
                    modelFile: shared_1.JOB_DEFINITIONS[dummySession.jobId]?.modelFile || 'Adventurer.glb'
                }
            }
        });
    ...
    despawnDummy(dummyId, session) {
        if (this.notFound(dummyId, session))
            return;
        const meta = this.dummyMeta.get(dummyId);
        if (meta.inParty) {
    ...
    setDummyProperty(dummyId, prop, value, session) {
        ...
        const propMap = {
            ...
            level: (v) => { dummy.stats.level = Math.max(1, Math.min(v, shared_1.MAX_LEVEL)); },
    ...
    setDummyClass(dummyId, jobIdStr, session) {
        ...
        if (!dummy)
            return;
        const validJobs = Object.values(shared_1.JobId);
        const targetJob = validJobs.find(j => j.toLowerCase() === jobIdStr.toLowerCase());
        if (!targetJob) {
            this.gm(session.characterId, `Invalid job "${jobIdStr}".`);
            return;
        }
        dummy.jobId = targetJob;
        dummy.baseClass = (0, shared_1.getBaseClassForJob)(targetJob);
        this.deps.recalcStats(dummy);
        this.deps.broadcastInZone(dummy.zoneId, {
            type: shared_1.PacketType.ENTITY_DESPAWN,
            timestamp: Date.now(),
        ...
        this.deps.broadcastInZone(dummy.zoneId, {
            ...
            data: {
                ...
                data: {
                    ...
                    modelFile: shared_1.JOB_DEFINITIONS[dummy.jobId]?.modelFile || 'Adventurer.glb'
                }
            }
        });
    ...
    setDummyGear(dummyId, preset, session) {
        ...
        for (const [slot, itemId] of Object.entries(gearPreset)) {
            if (itemId && this.deps.getItemDef(itemId)) {
                dummy.equipment[slot] = { itemId, enhancementLevel: 0, enhancementElement: null, quantity: 1, slot: 0 };
            }
            else {
                dummy.equipment[slot] = null;
            }
        }
        this.deps.recalcStats(dummy);
        this.deps.broadcastInZone(dummy.zoneId, {
    ...
    toggleDummyParty(dummyId, session) {
        ...
        if (meta.inParty) {
            ...
            if (!party) {
                party = partySys.createParty(session.characterId, session, { visibility: shared_1.PartyVisibility.OPEN, lootRule: shared_1.LootRule.RANDOM });
                if (!party) {
