...
async function startServer() {
    try {
        ...
        const networkServer = new NetworkServer_1.NetworkServer(httpServer, {
            redis: db.redis,
        ...
        const adminCandidates = [
            (0, path_1.join)(__dirname, 'core/apiManagementClient/dist'),
            (0, path_1.join)(__dirname, '../src/core/apiManagementClient/dist'),
            (0, path_1.join)(process.cwd(), 'src/core/apiManagementClient/dist'),
        ];
        const adminDist = adminCandidates.find(p => (0, fs_1.existsSync)(p));
        if (adminDist) {
            app.use('/admin', express_1.default.static(adminDist));
            app.get('/admin/*', (_req, res) => res.sendFile((0, path_1.join)(adminDist, 'index.html')));
        ...
        if (db.isRedisConnected()) {
            try {
                ...
                if (relaySub)
                    await relaySub.quit().catch(() => { });
                if (partySub)
                    await partySub.quit().catch(() => { });
                adapterPub = null;
                adapterSub = null;
                relaySub = null;
                partySub = null;
        ...
        function tick() {
            ...
            const elapsedMs = Number(now - lastTick) / 1000000;
            lastTick = now;
            networkServer.gameLoop(Math.min(elapsedMs, 200));
            const nextDelay = Math.max(0, tickInterval - Number(process.hrtime.bigint() - now) / 1000000);
