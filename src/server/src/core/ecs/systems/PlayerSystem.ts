...
export class PlayerSystem extends System {
   ...
   */
  private maintainObeliskBuff(session: PlayerSession): void {
    const values = getObeliskBuffForWeapon(this.itemSys, session.equipment?.weapon);
    const effects = session.statusEffects || [];
    const existing = effects.find(e => e.type === StatusEffectType.BUFF_OBELISK);
    ...
    if (values) {
      ...
      const buff = {
        ...
        type: StatusEffectType.BUFF_OBELISK,
      ...
      session.statusEffects = effects.filter(e => e.type !== StatusEffectType.BUFF_OBELISK);
  ...
  equipItem(session: PlayerSession, itemId: string): boolean {
    ...
    if (slot === EquipmentSlot.WEAPON && itemDef.obeliskBuff && session.statusEffects) {
      session.statusEffects = session.statusEffects.filter(
        e => e.type !== StatusEffectType.BUFF_GUARDED,
      );
    }
